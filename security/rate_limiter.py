...
def check_and_record_request(user_id: str) -> None:
    ...
    with db.get_conn() as conn:
        ...
        if recent >= RATE_LIMIT_PER_MINUTE:
            raise RateLimitExceeded(
                ...
                f"(limit: {RATE_LIMIT_PER_MINUTE}/minute). "
        ...
        if request_count >= DAILY_REQUEST_LIMIT:
            raise RateLimitExceeded(
                f"You've reached today's message limit "
                f"({DAILY_REQUEST_LIMIT}/day). "
                "It resets at midnight UTC — thanks for your patience!"
            )

        if total_tokens >= DAILY_TOKEN_LIMIT:
            raise RateLimitExceeded(
                f"You've reached today's usage limit "
                f"({DAILY_TOKEN_LIMIT:,} tokens/day). "
                "It resets at midnight UTC."
            )

        conn.execute(
            "INSERT INTO request_log (user_id, ts) "
            "VALUES (?, ?)",
            (user_id, now),
        )
    ...
    logger.info(
        f"Request recorded for user_id={user_id!r} "
        f"({request_count + 1}/{DAILY_REQUEST_LIMIT} today)"
    )


def get_usage_summary(user_id: str) -> dict:
    """Used by the sidebar to show today's usage."""

    today = _today()

    with db.get_conn() as conn:
        row = conn.execute(
            "SELECT request_count, prompt_tokens, "
